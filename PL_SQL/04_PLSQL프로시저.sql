...
CREATE PROCEDURE guguproc
    (p_dan IN NUMBER) -- 매개변수(IN은 프로시저 내부로 들어가는 값임을 의미)
...
EXEC p_test; --프로시저 호출문 (인수가 없을 땐 괄호 안 쓴다)

-- 익명 블록과 프로시저의 차이점
-- 1. 선언부 모양이 다르다
-- 2. 매개값을 받을 수 있다.
-- 3. 
-- 4. 익명블록은 사용할 때마다 드래그 f9해야 하는데 프로시저는 이름만 호출하면 된다.

-- IN 입력값을 여러개 전달 받는 프로시저
...
CREATE OR REPLACE PROCEDURE my_new_job_proc -- CREATE OR REPLACE
    (p_job_id IN jobs.job_id%TYPE,
     p_job_title IN jobs.job_title%TYPE,
     p_min_sal IN jobs.min_salary%TYPE,
     p_max_sal IN jobs.max_salary%TYPE
...
BEGIN
    ...
    --이미 존재한다면 1, 존재하지 않는다면 0 -> V-CNT에 저장
    SELECT
        COUNT(*)
    INTO v_cnt
    FROM jobs
...
EXEC my_new_job_proc('JOB2', '테스트 잡2', 10000, 13000);
